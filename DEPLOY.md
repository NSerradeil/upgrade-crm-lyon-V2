...
- **8.24.0 (23/09/2026)** : le scan des plateformes devient une CAMPAGNE. `agent_sequence_upsert` accepte `target_kind='plateforme'` (+ `cadence_jours`, `heure_passage`, `playbook_path`, `resume_at`, statut `paused`) ; nouveaux outils `agent_sequence_passage` (enregistrer UN passage de scan : étape atteinte, besoins vus/nouveaux, motif obligatoire sous le gate de couverture) et `agent_scan_stats` (stat par plateforme, matière de la proposition d'ajustement de cadence). `agent_sequence_due_today` rappelle d'abord les pauses arrivées à terme et couvre désormais les plateformes.
