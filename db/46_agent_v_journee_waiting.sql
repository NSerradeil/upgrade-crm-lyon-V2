-- 46_agent_v_journee_waiting.sql — 2026-09-21
-- « Attend ton GO » n'est pas « en cours » (constat Nicolas, 21/09).
--
-- Reprend db/43 à l'identique et ne change QUE le mapping de statut : `waiting_go` avait le
-- même état 'running' que `claimed`, donc la frise affichait une tâche qui ATTEND NICOLAS
-- avec la pastille bleue « en cours » et le texte « Jules travaille dessus — pas encore de
-- compte-rendu ». Cas vécu : « Validation fix envoi LK — campagne.envoyer HIRE-PARIS »,
-- en waiting_go depuis 18h09, lue comme un worker vivant pendant 5 h.
--
-- C'est l'inverse de ce qu'il faut lire : sur `claimed` il n'y a rien à faire, sur
-- `waiting_go` c'est Nicolas qui bloque la file. Les deux ont donc désormais leur état.
-- Additif : lecture seule, aucune autre colonne ni ligne ne change.
...
select o.mission_id, o.titre, o.prevu_at, 'routine'::text as famille,
       case
         ...
         when td.statut = 'waiting_go'               then 'waiting'
         when td.statut = 'claimed'                  then 'running'
...
-- Vérif :
--   select etat, count(*) from public.agent_v_journee group by etat;
--   select titre, etat from public.agent_v_journee where etat = 'waiting';
