-- 41_agent_v_journee_sequences.sql — 2026-09-21
-- Reprend db/40 et ajoute un 4e bloc : les relances de séquence du jour, au couloir campagnes.
...
-- Les relances de séquence (campagnes) ne sont pas des agent_tasks : sans cette branche,
-- « Sandy Jalouzot Cohen (étape 3) à 12h » s'affichait dans les échéances du jour mais
-- restait invisible sur la frise. On ne prend que les échéances DU JOUR : un retard de
-- trois semaines n'a pas de place sur une frise 7h→19h (il vit dans « en retard »).
select s.mission_id,
